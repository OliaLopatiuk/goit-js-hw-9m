...
function formSubmit(e) {
    ...
    let delay1 = Number(formField.elements.delay.value);
    const amount = Number(formField.elements.amount.value);
    const step = Number(formField.elements.step.value);
    console.log(delay1, step, amount);
    for(let i = 1; i <= amount; i += 1){
        createPromise(i, delay1).then(({ position , delay  })=>console.log(`✅ Fulfilled promise ${position} in ${delay}ms`)).catch(({ position , delay  })=>console.log(`❌ Rejected promise ${position} in ${delay}ms`));
        delay1 += step;
    }
}
